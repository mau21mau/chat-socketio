...
var connection = io.sockets.on('connection', function(socket) {
    ...
    socket.on('sendMessage', function (data) {
        ...
        var enviadaPara = usuarios[toSocketId].nome; //Obtem o nome de quem esta enviando
        ...
        var onesocket = io.sockets.socket(toSocketId); // obtem a instancia do socket da pessoa que ira receber a mensagem

        
        var mensagem = {message : menssagem_text, enviadoPor: enviadoPor, senderSocket : fromSocketId };
        onesocket.emit('recieveMessage', mensagem); // envia a mensagem
        console.log("Mensagem: '"+data.message+"' -> para '"+enviadaPara+"' | id-conexao: "+onesocket.id);
